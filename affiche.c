...
/*
An infinity loop to prompt the table on screen and once it end, we clean the terminal
for and easier reading.
*/
  while (steps!=1)
  {
    system("sleep 0.2");
    ...
    for(i = 0; i < N; i++)
    {
      ...
      for(j = 0; j < N; j++)
      {
        ...
        voisinage_count[i][j] = count_nbr (grid,i,j,N); // we're sending infos to count every neighbours in compteur.c
      }
    }
/*
We're now creating a new 2D Table with the results of voisinage_count with the rules of the GameOfLife.
If <=1 or >=4, we're changing the value of the position to 0, and if it's equal to 3 neighbours, we're modifying value
to 1. So the game can run without problems.
*/
    for ( i = 0; i < N; i++)
    {
      for ( j = 0; j < N; j++)
